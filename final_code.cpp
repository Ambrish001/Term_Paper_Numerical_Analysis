...
double df(double y, int a)//This function is useful in reducing the length of terms used in Adomian Polynomials
{
	double b=1;
	for(int i=0;i<a;i++)
	{
...
double masterpoly(double x, double u[], int order)//This function returns the value of Adomian polynomial of specified order
{
...
//This function uses the Adomian Decomposition Method(ADM) for estimating value of z
double ADM(double temp, double pres, double yg, double yn, double yco, double yhs, double tpc, double ppc, double tpr, double ppr,double a)
{
	double tr;
	if(yg!=-1)
	{
    ...
    if(a==-1)
    {
        cout<<"tpc = "<<tpc<<" ppc = "<<ppc<<" tr = "<<tr<<" ppr = "<<ppr<<" \n";
...
//This function uses the ADM+Shanks transform for estimating value of z
double ADM_Shanks(double temp, double pres, double yg, double yn, double yco, double yhs, double tpc, double ppc, double tpr, double ppr, double a)
{
	double tr;
	if(yg!=-1)
	{
	...
	if(a==-1)
	{
	    ...
	    for(int i=0;i<3;i++)
	    {
	        cout<<"Sh(U"<<i+1<<") = "<<yshank[i]<<"\n";
	    }
	    cout<<"Sh(Sh(U2)) = "<<yshank[3]<<"\n";
	}
...
//This function uses the Newton Raphson method for estimating value of z
double Newton(double temp, double pres, double yg, double yn, double yco, double yhs, double tpc, double ppc, double tpr, double ppr,double ig)
{
	...
	double tolerance=0.0000000001,error,xprev,xnew,fx,fxi,counter=0,cons;
...
int main()
{
	...
	double tr[4]={1.3,1.5,1.7,2},pr[7]={0.5,1.5,2.5,3.5,4.5,5.5,6.5},count=4,a=0,zn=0,za=0,zs=0;
	for(int i=0;i<2;i++)
	{
		cout<<"For test case "<<i+1<<":\n";
		a=Newton(temper[i],press[i],yg[i],yn[i],yc[i],yh[i],-1,-1,-1,-1,0);
		a=Newton(temper[i],press[i],yg[i],yn[i],yc[i],yh[i],-1,-1,-1,-1,0.5);
		a=Newton(temper[i],press[i],yg[i],yn[i],yc[i],yh[i],-1,-1,-1,-1,0.8);
		a=Newton(temper[i],press[i],yg[i],yn[i],yc[i],yh[i],-1,-1,-1,-1,0.9);
		a=Newton(temper[i],press[i],yg[i],yn[i],yc[i],yh[i],-1,-1,-1,-1,1);
		a=Newton(temper[i],press[i],yg[i],yn[i],yc[i],yh[i],-1,-1,-1,-1,1.2);
		a=Newton(temper[i],press[i],yg[i],yn[i],yc[i],yh[i],-1,-1,-1,-1,1.3);
		a=Newton(temper[i],press[i],yg[i],yn[i],yc[i],yh[i],-1,-1,-1,-1,1.5);
		a=ADM(temper[i],press[i],yg[i],yn[i],yc[i],yh[i],-1,-1,-1,-1,-1);
		a=ADM_Shanks(temper[i],press[i],yg[i],yn[i],yc[i],yh[i],-1,-1,-1,-1,-1);
		cout<<"--------------------------------\n";
	}

	cout<<"For test case 3:\n";
	a=Newton(temper[2],press[2],-1,-1,-1,-1,237.2,4.481592,-1,-1,0.5);
	a=ADM(temper[2],press[2],-1,-1,-1,-1,237.2,4.481592,-1,-1,-1);
	a=ADM_Shanks(temper[2],press[2],-1,-1,-1,-1,237.2,4.481592,-1,-1,-1);
	cout<<"--------------------------------\n";

	//This the experimental data as obtained from the Standing Katz table
	double zexp[4][7] = {{0.916,0.756,0.638,0.633,0.684,0.759,0.844},{0.948,0.859,0.794,0.770,0.790,0.836,0.892},{0.968,0.914,0.876,0.857,0.864,0.897,0.942},{0.982,0.956,0.941,0.937,0.945,0.969,0.986}};

	for(int i=0;i<4;i++)
	{
		for(int j=0;j<7;j++)
		{
			cout<<"For test case "<<count<<":\n";
			zn=Newton(-1,-1,-1,-1,-1,-1,-1,-1,tr[i],pr[j],0.8);
			za=ADM(-1,-1,-1,-1,-1,-1,-1,-1,tr[i],pr[j],0);
			zs=ADM_Shanks(-1,-1,-1,-1,-1,-1,-1,-1,tr[i],pr[j],0);
			cout<<"Absolute Relative Deviation of z for Newton-Raphson method: "<<abs(zexp[i][j]-zn)/zexp[i][j]<<"\n";
			cout<<"Absolute Relative Deviation of z for ADM method: "<<abs(zexp[i][j]-za)/zexp[i][j]<<"\n";
			cout<<"Absolute Relative Deviation of z for ADM-Shanks method: "<<abs(zexp[i][j]-zs)/zexp[i][j]<<"\n";
			cout<<"--------------------------------\n";
			count+=1;
		}
	}
